...
const MobileHeader = () => {
      ...
      return (
            <section className=" fixed z-100 lg:hidden top-0 flex items-center justify-between w-full py-4 px-5 bg-background ">
                  <Logo />
                  <button aria-label='Boton menu mobil' onClick={setIsActiveMenu} className=" flex justify-center items-center w-10 h-10 bg-cream border border-border/20 rounded-xl cursor-pointer ">
                        <BsList size={22} />
                  </button>
            </section>
      )
